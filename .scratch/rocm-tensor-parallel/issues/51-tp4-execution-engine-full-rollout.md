...
- [x] All 43 layers (plus output head/embedding) run under the persistent
      per-rank thread model with stream/event-based dispatch — no remaining
      `hipSetDevice` churn in steady-state decode
- [x] Per-call-site sync counts (via #49's harness) confirm the reduction
      generalizes from the #50 spike to the full model, not just the spiked
      subset
- [x] End-to-end TP=4 decode throughput measured and compared against both
