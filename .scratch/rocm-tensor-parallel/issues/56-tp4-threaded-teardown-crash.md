...
- [ ] Root cause of the "Memobj map does not have ptr" teardown abort
      identified with a minimal repro or a clear causal chain, not just a
      plausible reading of the code (the mistake #50 made)
- [ ] Fix implemented; the same real-hardware threaded generation that
      crashed in #51 (2 layers, `-n 40`) completes and exits cleanly
- [ ] `make -j8 cpu`, `make -j8 rocm`, `make -j8 test-rocm` all pass
