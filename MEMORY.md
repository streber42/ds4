...
- **TP=4 Prefill NaN Resolution** — Fixed activation pointer overwrite bug in `routed_moe_launch`, negative expert slot indexing (`compact_i < 0`), and prefill fallback VRAM cleanup; `case_000` evaluates clean with `nan_cnt = 0`